...
export default function InfoBar() {
  ...
  useEffect(() => {
    ...
    const cachedForex = localStorage.getItem('tz_forex');
    const cachedForexTime = localStorage.getItem('tz_forex_time');
      ...
      fetch('https://open.er-api.com/v6/latest/USD')
        ...
        .then(data => {
          if (data?.rates?.PHP) {
            ...
            localStorage.setItem('tz_forex', rate);
            localStorage.setItem('tz_forex_time', String(Date.now()));
    ...
    const cachedTemp = localStorage.getItem('tz_temp');
    const cachedTempTime = localStorage.getItem('tz_temp_time');
    ...
    if (
      ...
      fetch(
        'https://api.open-meteo.com/v1/forecast?latitude=14.6833&longitude=120.8833&current_weather=true'
      )
        .then(r => r.json())
        .then(data => {
          if (data?.current_weather?.temperature !== undefined) {
            const t = `${Math.round(data.current_weather.temperature)}°C`;
            localStorage.setItem('tz_temp', t);
            localStorage.setItem('tz_temp_time', String(Date.now()));
    ...
    <div className="bg-primary-900 text-white text-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 flex items-center justify-end gap-6">
        ...
        <span className="hidden sm:flex items-center gap-1.5 opacity-90">
          ...
          <span className="text-gray-300">Tanza</span>
